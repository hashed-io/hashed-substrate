...
fn cancel_approval_works() {
  new_test_ext().execute_with(|| {
    ...
    assert_ok!(Assets::approve_transfer(RuntimeOrigin::signed(1), 0, 2, 50));
    assert_eq!(Asset::<Test>::get(0).unwrap().approvals, 1);
    assert_noop!(Assets::cancel_approval(RuntimeOrigin::signed(1), 1, 2), Error::<Test>::Unknown);
    assert_noop!(Assets::cancel_approval(RuntimeOrigin::signed(2), 0, 2), Error::<Test>::Unknown);
    assert_noop!(Assets::cancel_approval(RuntimeOrigin::signed(1), 0, 3), Error::<Test>::Unknown);
    assert_eq!(Asset::<Test>::get(0).unwrap().approvals, 1);
    assert_ok!(Assets::cancel_approval(RuntimeOrigin::signed(1), 0, 2));
    assert_eq!(Asset::<Test>::get(0).unwrap().approvals, 0);
    assert_noop!(Assets::cancel_approval(RuntimeOrigin::signed(1), 0, 2), Error::<Test>::Unknown);
  });
}
...
fn non_providing_should_work() {
  new_test_ext().execute_with(|| {
    ...
    assert_noop!(Assets::mint(RuntimeOrigin::signed(1), 0, 1, 100), TokenError::CannotCreate);
    // ...or transfer...
    assert_noop!(Assets::transfer(RuntimeOrigin::signed(0), 0, 1, 50), TokenError::CannotCreate);
    // ...or force-transfer
    assert_noop!(
...
fn transferring_frozen_asset_should_not_work() {
  new_test_ext().execute_with(|| {
    ...
    assert_eq!(Assets::balance(0, 1), 100);
    assert_ok!(Assets::freeze_asset(RuntimeOrigin::signed(1), 0));
    assert_noop!(Assets::transfer(RuntimeOrigin::signed(1), 0, 2, 50), Error::<Test>::AssetNotLive);
...
fn origin_guards_should_work() {
  new_test_ext().execute_with(|| {
    ...
    assert_noop!(Assets::mint(RuntimeOrigin::signed(2), 0, 2, 100), Error::<Test>::NoPermission);
    assert_noop!(Assets::burn(RuntimeOrigin::signed(2), 0, 1, 100), Error::<Test>::NoPermission);
    assert_noop!(
      Assets::force_transfer(RuntimeOrigin::signed(2), 0, 1, 2, 100),
      Error::<Test>::NoPermission
    );
    assert_noop!(Assets::start_destroy(RuntimeOrigin::signed(2), 0), Error::<Test>::NoPermission);
  });
}
...
fn transfer_owner_should_work() {
  new_test_ext().execute_with(|| {
    ...
    assert_ok!(Assets::set_metadata(RuntimeOrigin::signed(2), 0, vec![0u8; 10], vec![0u8; 10], 12));
...
fn transferring_amount_more_than_available_balance_should_not_work() {
  new_test_ext().execute_with(|| {
    ...
    assert_eq!(Assets::balance(0, 1), 0);
    assert_noop!(Assets::transfer(RuntimeOrigin::signed(1), 0, 1, 50), Error::<Test>::NoAccount);
    assert_noop!(Assets::transfer(RuntimeOrigin::signed(2), 0, 1, 51), Error::<Test>::BalanceLow);
  });
}
...
fn transferring_more_units_than_total_supply_should_not_work() {
  new_test_ext().execute_with(|| {
    ...
    assert_ok!(Assets::mint(RuntimeOrigin::signed(1), 0, 1, 100));
    assert_eq!(Assets::balance(0, 1), 100);
    assert_noop!(Assets::transfer(RuntimeOrigin::signed(1), 0, 2, 101), Error::<Test>::BalanceLow);
  });
}
...
fn burning_asset_balance_with_zero_balance_does_nothing() {
  new_test_ext().execute_with(|| {
    ...
    assert_noop!(Assets::burn(RuntimeOrigin::signed(1), 0, 2, u64::MAX), Error::<Test>::NoAccount);
    assert_eq!(Assets::balance(0, 2), 0);
...
fn set_metadata_should_work() {
  new_test_ext().execute_with(|| {
    ...
    assert_ok!(Assets::set_metadata(RuntimeOrigin::signed(1), 0, vec![0u8; 10], vec![0u8; 10], 12));
    assert_eq!(Balances::free_balance(&1), 9);

    // Update deposit
    assert_ok!(Assets::set_metadata(RuntimeOrigin::signed(1), 0, vec![0u8; 10], vec![0u8; 5], 12));
    assert_eq!(Balances::free_balance(&1), 14);
    assert_ok!(Assets::set_metadata(RuntimeOrigin::signed(1), 0, vec![0u8; 10], vec![0u8; 15], 12));
    ...
    // Clear Metadata
    assert!(Metadata::<Test>::contains_key(0));
    assert_noop!(Assets::clear_metadata(RuntimeOrigin::signed(2), 0), Error::<Test>::NoPermission);
...
fn freezer_should_work() {
  new_test_ext().execute_with(|| {
    ...
    assert_noop!(Assets::transfer(RuntimeOrigin::signed(1), 0, 2, 21), Error::<Test>::BalanceLow);
...
fn force_metadata_should_work() {
  new_test_ext().execute_with(|| {
    ...
    assert_noop!(
      Assets::force_set_metadata(RuntimeOrigin::root(), 1, vec![0u8; 10], vec![0u8; 10], 8, false),
      Error::<Test>::Unknown
    );
    ...
    assert_noop!(Assets::force_clear_metadata(RuntimeOrigin::root(), 1), Error::<Test>::Unknown);
  });
}
...
fn force_asset_status_should_work() {
  new_test_ext().execute_with(|| {
    ...
    assert_ok!(Assets::force_asset_status(RuntimeOrigin::root(), 0, 1, 1, 1, 1, 100, true, false));
    assert_eq!(Assets::balance(0, 1), 50);
    ...
    assert_ok!(Assets::force_asset_status(RuntimeOrigin::root(), 0, 1, 1, 1, 1, 110, true, false));
...
#[test]
fn reserve_equal_to_min_balance_should_work() {
  new_test_ext().execute_with(|| {
    assert_ok!(Assets::force_create(RuntimeOrigin::root(), 0, 1, true, 1));
    assert_ok!(Assets::mint(RuntimeOrigin::signed(1), 0, 2, 100));
    assert_eq!(Assets::balance(0, 2), 100);
    assert_ok!(Assets::reserve(RuntimeOrigin::signed(1), 0, 0, 2, 1));
    assert_eq!(System::events().len(), 4);
    assert_eq!(
      System::events()[3].event,
      RuntimeEvent::Assets(Event::<Test>::Reserved {
        reserve_id: 0,
        asset_id: 0,
        who: 2,
        amount: 1
    ...
    assert_eq!(Assets::balance(0, 2), 99);
    assert_eq!(Assets::reserved_balance(0, 2), 1);
    assert_eq!(Assets::total_supply(0), 100);
    assert_eq!(Assets::total_supply(0), 99);
...
#[test]
fn reserve_without_admin_permission_should_fail() {
  new_test_ext().execute_with(|| {
    assert_ok!(Assets::force_create(RuntimeOrigin::root(), 0, 1, true, 1));
    assert_ok!(Assets::mint(RuntimeOrigin::signed(1), 0, 2, 100));
    assert_eq!(Assets::balance(0, 2), 100);
    assert_noop!(
      Assets::reserve(RuntimeOrigin::signed(2), 0, 0, 2, 1),
      Error::<Test>::NoPermission
    );
  });
}
...
#[test]
fn reserve_with_non_existant_account_should_fail() {
  new_test_ext().execute_with(|| {
    assert_ok!(Assets::force_create(RuntimeOrigin::root(), 0, 1, true, 1));
    assert_noop!(Assets::reserve(RuntimeOrigin::signed(1), 0, 0, 2, 1), Error::<Test>::NoAccount);
  });
}

#[test]
fn reserve_lower_than_min_balance_should_fail() {
  new_test_ext().execute_with(|| {
    assert_ok!(Assets::force_create(RuntimeOrigin::root(), 0, 1, true, 2));
    assert_ok!(Assets::mint(RuntimeOrigin::signed(1), 0, 2, 100));
    assert_eq!(Assets::balance(0, 2), 100);
    assert_noop!(Assets::reserve(RuntimeOrigin::signed(1), 0, 0, 2, 1), TokenError::BelowMinimum);
  });
}

#[test]
fn reserve_greater_than_balance_should_fail() {
  new_test_ext().execute_with(|| {
    assert_ok!(Assets::force_create(RuntimeOrigin::root(), 0, 1, true, 2));
    assert_ok!(Assets::mint(RuntimeOrigin::signed(1), 0, 2, 100));
    assert_eq!(Assets::balance(0, 2), 100);
    assert_noop!(
      Assets::reserve(RuntimeOrigin::signed(1), 0, 0, 2, 101),
      Error::<Test>::BalanceLow
    );
  });
}
...
#[test]
fn reserve_with_existing_identifier_should_fail() {
  new_test_ext().execute_with(|| {
    assert_ok!(Assets::force_create(RuntimeOrigin::root(), 0, 1, true, 2));
    assert_ok!(Assets::mint(RuntimeOrigin::signed(1), 0, 2, 100));
    assert_eq!(Assets::balance(0, 2), 100);
    assert_ok!(Assets::reserve(RuntimeOrigin::signed(1), 0, 0, 2, 10));
    assert_noop!(
      Assets::reserve(RuntimeOrigin::signed(1), 0, 0, 2, 3),
      Error::<Test>::ReserveAlreadyExists
    );
  });
}

#[test]
fn reserve_greater_than_balance_should_fail2() {
  new_test_ext().execute_with(|| {
    assert_ok!(Assets::force_create(RuntimeOrigin::root(), 0, 1, true, 2));
    assert_ok!(Assets::mint(RuntimeOrigin::signed(1), 0, 2, 100));
    assert_eq!(Assets::balance(0, 2), 100);
    assert_ok!(Assets::reserve(RuntimeOrigin::signed(1), 0, 0, 2, 10));
    assert_noop!(Assets::reserve(RuntimeOrigin::signed(1), 0, 0, 2, 91), Error::<Test>::BalanceLow);
  });
}
...
fn unreserve_should_work() {
  new_test_ext().execute_with(|| {
    ...
    assert_ok!(Assets::mint(RuntimeOrigin::signed(1), 0, 2, 100));
    assert_eq!(Assets::balance(0, 2), 100);
    assert_ok!(Assets::reserve(RuntimeOrigin::signed(1), 0, 0, 2, 2));
    assert_eq!(Assets::balance(0, 2), 98);
    assert_eq!(Assets::reserved_balance(0, 2), 2);
    assert_eq!(Assets::total_supply(0), 100);
    assert_eq!(Assets::total_reserved_supply(0), 2);
    assert_eq!(Assets::total_supply(0), 98);
    ...
    assert_eq!(
      ...
      RuntimeEvent::Assets(Event::<Test>::Unreserved {
        reserve_id: 0,
        asset_id: 0,
        who: 2,
        amount: 2
...
#[test]
fn unreserve_without_admin_permission_should_fail() {
  new_test_ext().execute_with(|| {
    assert_ok!(Assets::force_create(RuntimeOrigin::root(), 0, 1, true, 1));
    assert_ok!(Assets::mint(RuntimeOrigin::signed(1), 0, 2, 100));
    assert_eq!(Assets::balance(0, 2), 100);
    assert_ok!(Assets::reserve(RuntimeOrigin::signed(1), 0, 0, 2, 2));
    assert_noop!(Assets::unreserve(RuntimeOrigin::signed(2), 0, 0, 2), Error::<Test>::NoPermission);
  });
}

#[test]
fn unreserve_for_non_existant_reserve_should_fail() {
  new_test_ext().execute_with(|| {
    assert_ok!(Assets::force_create(RuntimeOrigin::root(), 0, 1, true, 1));
    assert_ok!(Assets::mint(RuntimeOrigin::signed(1), 0, 2, 100));
    assert_eq!(Assets::balance(0, 2), 100);
    assert_noop!(
      Assets::unreserve(RuntimeOrigin::signed(1), 0, 0, 2),
      Error::<Test>::UnknownReserve
    );
  });
}

#[test]
fn burn_reserve_should_work() {
  new_test_ext().execute_with(|| {
    assert_ok!(Assets::force_create(RuntimeOrigin::root(), 0, 1, true, 1));
    assert_ok!(Assets::mint(RuntimeOrigin::signed(1), 0, 2, 100));
    assert_eq!(Assets::balance(0, 2), 100);
    ...
    assert_eq!(
      System::events()[4].event,
      RuntimeEvent::Assets(Event::<Test>::BurnedReserve {
        reserve_id: 0,
        asset_id: 0,
        who: 2,
        amount: 10
...
#[test]
fn burn_reserve_without_admin_permission_should_fail() {
  new_test_ext().execute_with(|| {
    assert_ok!(Assets::force_create(RuntimeOrigin::root(), 0, 1, true, 1));
    assert_ok!(Assets::mint(RuntimeOrigin::signed(1), 0, 2, 100));
    assert_eq!(Assets::balance(0, 2), 100);
    assert_ok!(Assets::reserve(RuntimeOrigin::signed(1), 0, 0, 2, 2));
    assert_noop!(
      Assets::burn_reserve(RuntimeOrigin::signed(2), 0, 0, 2),
      Error::<Test>::NoPermission
    );
  });
}
...
#[test]
fn burn_reserve_for_non_existant_reserve_should_fail() {
  new_test_ext().execute_with(|| {
    assert_ok!(Assets::force_create(RuntimeOrigin::root(), 0, 1, true, 1));
    assert_ok!(Assets::mint(RuntimeOrigin::signed(1), 0, 2, 100));
    assert_eq!(Assets::balance(0, 2), 100);
    assert_noop!(
      Assets::burn_reserve(RuntimeOrigin::signed(1), 0, 0, 2),
      Error::<Test>::UnknownReserve
    );
  });
}
...
fn transfer_named_reserve_to_new_account_should_work() {
  new_test_ext().execute_with(|| {
    ...
    assert_eq!(
      ...
      RuntimeEvent::Assets(Event::<Test>::TransferredReserve {
        reserve_id: 0,
        asset_id: 0,
        from: 2,
        to: 3,
    ...
    assert_eq!(Assets::balance(0, 2), 80);
    assert_eq!(Assets::reserved_balance(0, 2), 0);
    assert_eq!(Assets::balance(0, 3), 20);
...
fn transfer_named_reserve_for_non_existant_reserve_should_fail() {
  new_test_ext().execute_with(|| {
    ...
    assert_eq!(Assets::balance(0, 2), 100);
    assert_noop!(
      Assets::transfer_named_reserve(&0, 0, &2, &3, None),
      Error::<Test>::UnknownReserve
    );
  });
}
