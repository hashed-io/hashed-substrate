...
fn basic_minting_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Assets::free_balance(0, 1), 100);
...
fn refunding_asset_deposit_with_burn_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Assets::free_balance(1, 0), 0);
	});
}
...
fn refunding_asset_deposit_without_burn_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Assets::transfer(RuntimeOrigin::signed(1), 0, 2, 100));
		assert_eq!(Assets::free_balance(0, 2), 100);
		assert_eq!(Assets::free_balance(0, 1), 0);
		assert_eq!(Balances::reserved_balance(&1), 10);
...
fn approval_lifecycle_works() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Assets::free_balance(0, 1), 60);
		assert_eq!(Assets::free_balance(0, 3), 40);
		assert_eq!(Balances::reserved_balance(&1), 0);
	});
}
...
fn transfer_approved_all_funds() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Asset::<Test>::get(0).unwrap().approvals, 0);
		assert_eq!(Assets::free_balance(0, 1), 50);
		assert_eq!(Assets::free_balance(0, 3), 50);
		assert_eq!(Balances::reserved_balance(&1), 0);
	});
}
...
fn min_balance_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert!(Assets::maybe_free_balance(0, 1).is_none());
		assert_eq!(Assets::free_balance(0, 2), 100);
		assert_eq!(Asset::<Test>::get(0).unwrap().accounts, 1);
		...
		assert!(Assets::maybe_free_balance(0, 2).is_none());
		assert_eq!(Assets::free_balance(0, 1), 100);
		assert_eq!(Asset::<Test>::get(0).unwrap().accounts, 1);
...
fn querying_total_supply_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Assets::free_balance(0, 1), 50);
		assert_eq!(Assets::free_balance(0, 2), 50);
		assert_ok!(Assets::transfer(RuntimeOrigin::signed(2), 0, 3, 31));
		assert_eq!(Assets::free_balance(0, 1), 50);
		assert_eq!(Assets::free_balance(0, 2), 19);
		assert_eq!(Assets::free_balance(0, 3), 31);
...
fn transferring_enough_to_kill_source_when_keep_alive_should_fail() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Assets::free_balance(0, 1), 10);
		assert_eq!(Assets::free_balance(0, 2), 90);
...
fn transferring_to_frozen_account_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Assets::transfer(RuntimeOrigin::signed(1), 0, 2, 50));
		assert_eq!(Assets::free_balance(0, 2), 150);
	});
}
...
fn burning_asset_balance_with_zero_balance_does_nothing() {
	new_test_ext().execute_with(|| {
		assert_ok!(Assets::force_create(RuntimeOrigin::root(), 0, 1, true, 1));
		assert_ok!(Assets::mint(RuntimeOrigin::signed(1), 0, 1, 100));
		assert_eq!(Assets::free_balance(0, 2), 0);
		assert_noop!(
...
fn set_metadata_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(Assets::clear_metadata(RuntimeOrigin::signed(1), 1), Error::<Test>::UnknownAsset);
...
fn imbalances_should_work() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Assets::free_balance(0, 1), 30);
		assert_eq!(Assets::total_supply(0), 30);
	});
}
...
fn force_asset_status_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Assets::free_balance(0, 1), 51);
		...
		assert_eq!(Assets::free_balance(0, 1), 200);
		assert_eq!(Assets::free_balance(0, 2), 0);
...
fn assets_from_genesis_should_exist() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Assets::free_balance(999, 1), 100);
...
fn reserve_equal_to_min_balance_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(System::events()[3].event, RuntimeEvent::Assets(Event::<Test>::Reserved{
			reserve_id: 0,
			asset_id: 0,
			who: 2,
			amount: 1
...
fn reserve_without_admin_permission_should_fail() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Assets::mint(RuntimeOrigin::signed(1), 0, 2, 100));
		assert_eq!(Assets::free_balance(0, 2), 100);
		assert_noop!(Assets::reserve(RuntimeOrigin::signed(2), 0, 0, 2, 1), Error::<Test>::NoPermission);
	});
}
...
fn reserve_greater_than_balance_should_fail() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Assets::mint(RuntimeOrigin::signed(1), 0, 2, 100));
		assert_eq!(Assets::free_balance(0, 2), 100);
		assert_noop!(Assets::reserve(RuntimeOrigin::signed(1), 0, 0, 2, 101), Error::<Test>::BalanceLow);
	});
}
...
fn reserve_with_existing_identifier_should_fail() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(Assets::reserve(RuntimeOrigin::signed(1), 0, 0, 2, 3), Error::<Test>::ReserveAlreadyExists);
	});
}

#[test]
fn reserve_greater_than_free_balance_should_fail() {
	new_test_ext().execute_with(|| {
		assert_ok!(Assets::force_create(RuntimeOrigin::root(), 0, 1, true, 2));
		assert_ok!(Assets::mint(RuntimeOrigin::signed(1), 0, 2, 100));
		assert_eq!(Assets::free_balance(0, 2), 100);
...
fn unreserve_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Assets::free_balance(0, 2), 98);
		...
		assert_eq!(System::events()[4].event, RuntimeEvent::Assets(Event::<Test>::Unreserved{
			reserve_id: 0,
			asset_id: 0,
			who: 2,
			amount: 2
...
fn unreserve_for_non_existant_reserve_should_fail() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Assets::mint(RuntimeOrigin::signed(1), 0, 2, 100));
		assert_eq!(Assets::free_balance(0, 2), 100);
		assert_noop!(Assets::unreserve(RuntimeOrigin::signed(1), 0, 0, 2), Error::<Test>::UnknownReserve);
	});
}
...
fn burn_reserve_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(System::events().len(), 5);
		assert_eq!(System::events()[4].event, RuntimeEvent::Assets(Event::<Test>::BurnedReserve {
			reserve_id: 0,
			asset_id: 0,
			who: 2,
			amount: 10
...
fn burn_reserve_without_admin_permission_should_fail() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Assets::free_balance(0, 2), 100);
		assert_ok!(Assets::reserve(RuntimeOrigin::signed(1), 0, 0, 2, 2));
		assert_noop!(Assets::burn_reserve(RuntimeOrigin::signed(2), 0, 0, 2), Error::<Test>::NoPermission);
	});
}
