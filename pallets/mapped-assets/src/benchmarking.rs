...
fn create_default_minted_asset<T: Config<I>, I: 'static>(
	...
	if !is_sufficient {
		T::Currency::make_balance_be(&caller, T::Currency::minimum_balance());
	}
...
fn add_consumers<T: Config<I>, I: 'static>(minter: T::AccountId, n: u32) {
	...
	for i in 0..n {
		...
		T::Currency::make_balance_be(&target, T::Currency::minimum_balance());
...
benchmarks_instance_pallet! {
	create {
		...
		let caller_lookup = T::Lookup::unlookup(caller.clone());
		T::Currency::make_balance_be(&caller, DepositBalanceOf::<T, I>::max_value());
	...
	transfer_approved {
		...
		T::Currency::make_balance_be(&owner, DepositBalanceOf::<T, I>::max_value());
