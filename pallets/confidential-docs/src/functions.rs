...
impl<T: Config> Pallet<T> {
  ...
  pub fn do_add_group_member(
    ...
    ensure!(!GroupMembers::<T>::contains_key(group, member), Error::<T>::UserAlreadyGroupMember);
    Self::store_group_member(group_member.clone())?;
  ...
  pub fn do_remove_group_member(
    ...
    let group_member =
      GroupMembers::<T>::get(&group, &member).ok_or_else(|| Error::<T>::GroupMemberDoesNotExist)?;
