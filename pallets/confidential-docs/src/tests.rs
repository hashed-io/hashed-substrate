...
fn remove_owned_document_should_fail_for_not_document_owner() {
  new_test_ext().execute_with(|| {
    ...
    let not_owner = 2;
    assert_noop!(
      ConfidentialDocs::remove_owned_document(RuntimeOrigin::signed(not_owner), doc1.cid.clone()),
      Error::<Test>::NotDocOwner
    );
  });
}
...
fn remove_shared_document_should_fail_for_non_existant_doc() {
  new_test_ext().execute_with(|| {
    ...
    let shared_doc1 = generate_shared_doc("1", from, to);
    assert_noop!(
      ConfidentialDocs::remove_shared_document(RuntimeOrigin::signed(to), shared_doc1.cid.clone()),
      Error::<Test>::DocNotFound
    );
  });
}
...
fn remove_shared_document_should_fail_for_not_doc_sharee() {
  new_test_ext().execute_with(|| {
    ...
    let shared_doc1 = setup_shared_doc("1", from, to1);
    assert_noop!(
      ConfidentialDocs::remove_shared_document(RuntimeOrigin::signed(to2), shared_doc1.cid.clone()),
      Error::<Test>::NotDocSharee
    );
    assert_noop!(
...
fn add_group_member_should_fail_for_non_existant_group() {
  new_test_ext().execute_with(|| {
    ...
    assert_noop!(
      ConfidentialDocs::add_group_member(RuntimeOrigin::signed(creator), group_member.clone()),
...
fn add_group_member_should_fail_for_authorizer_not_member_of_group() {
  new_test_ext().execute_with(|| {
    ...
    assert_noop!(
      ConfidentialDocs::add_group_member(RuntimeOrigin::signed(non_creator), group_member.clone()),
...
fn add_group_member_should_fail_for_authorizer_not_admin() {
  new_test_ext().execute_with(|| {
    ...
    assert_noop!(
      ConfidentialDocs::add_group_member(RuntimeOrigin::signed(non_admin), group_member.clone()),
      Error::<Test>::NoPermission
    );
  });
}
...
fn remove_group_member_should_fail_for_trying_to_remove_non_member() {
  new_test_ext().execute_with(|| {
    ...
    assert_noop!(
      ConfidentialDocs::remove_group_member(RuntimeOrigin::signed(creator), group, non_member),
