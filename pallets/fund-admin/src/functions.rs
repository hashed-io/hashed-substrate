...
impl<T: Config> Pallet<T> {
	...
	pub fn do_initial_setup() -> DispatchResult {
		...
		let builder_role_id = T::Rbac::create_and_set_roles(
			pallet_id.clone(),
			[ProxyRole::Builder.to_vec()].to_vec(),
		)?;
		T::Rbac::create_and_set_permissions(
			pallet_id.clone(),
			builder_role_id[0],
		...
		let investor_role_id = T::Rbac::create_and_set_roles(
			pallet_id.clone(),
			[ProxyRole::Investor.to_vec()].to_vec(),
		)?;
		T::Rbac::create_and_set_permissions(
			pallet_id.clone(),
			investor_role_id[0],
		...
		let issuer_role_id = T::Rbac::create_and_set_roles(
			pallet_id.clone(),
			[ProxyRole::Issuer.to_vec()].to_vec(),
		)?;
		T::Rbac::create_and_set_permissions(
			pallet_id.clone(),
			issuer_role_id[0],
	...
	pub fn do_create_project(
		...
		let project_data = ProjectData::<T> {
			...
			regional_center: Some(
				BoundedVec::<T::AccountId, T::MaxRegionalCenterPerProject>::default(),
			),
			title,
			description,
			image,
	...
	pub fn do_delete_project(admin: T::AccountId, project_id: ProjectId) -> DispatchResult {
		...
		if UsersByProject::<T>::contains_key(project_id) {
			...
			for user in users_by_project.iter().cloned() {
				// Get user data
				let user_data = <UsersInfo<T>>::try_get(user.clone())
					.map_err(|_| Error::<T>::UserNotRegistered)?;
	...
	fn do_update_expenditure(
		...
		<ExpendituresInfo<T>>::try_mutate::<_, _, DispatchError, _>(
			...
			|expenditure_data| {
				let expenditure =
					expenditure_data.as_mut().ok_or(Error::<T>::ExpenditureNotFound)?;
	...
	pub fn do_reject_drawdown(
		...
		match drawdown_data.drawdown_type {
			...
			_ => {
				...
				<DrawdownsInfo<T>>::try_mutate::<_, _, DispatchError, _>(
					drawdown_id,
					|drawdown_data| {
						let drawdown_data =
							drawdown_data.as_mut().ok_or(Error::<T>::DrawdownNotFound)?;
			...
			match project.2 {
				CUDAction::Create => {
					...
					// Ensure project has no inflation rate
						...
						project_data.inflation_rate.is_none(),
						Error::<T>::InflationRateAlreadySet
					);

					// Set inflation rate
					<ProjectsInfo<T>>::try_mutate::<_, _, DispatchError, _>(
						project.0,
						|project_info| {
							let mod_project_data =
								project_info.as_mut().ok_or(Error::<T>::ProjectNotFound)?;
	...
	fn do_update_job_eligible(
		...
		<JobEligiblesInfo<T>>::try_mutate::<_, _, DispatchError, _>(
			...
			|job_eligible_data| {
				let job_eligible =
					job_eligible_data.as_mut().ok_or(Error::<T>::JobEligibleNotFound)?;
	...
	fn do_delete_job_eligible(job_eligible_id: JobEligibleId) -> DispatchResult {
		...
		Self::deposit_event(Event::JobEligibleDeleted(
			job_eligible_data.project_id,
			job_eligible_id,
		));
		...
		<RevenueTransactionsInfo<T>>::try_mutate::<_, _, DispatchError, _>(
			...
			|revenue_transaction_data| {
				...
				// Ensure job eligible exists
					...
					JobEligiblesInfo::<T>::contains_key(
						mod_revenue_transaction_data.job_eligible_id
					),
					Error::<T>::JobEligibleNotFound
				);
		...
		match action {
			CUDAction::Create => {
				...
				// Ensure confirming documents are not empty
					...
					!mod_confirming_documents.is_empty(),
					Error::<T>::BankConfirmingDocumentsEmpty
				);

				// Create drawdown bank confirming documents
				Self::do_create_bank_confirming_documents(
					project_id,
					drawdown_id,
					...
					match project.builder.as_mut() {
						...
						None => {
							let devs = project
								.builder
								.get_or_insert(
									BoundedVec::<T::AccountId, T::MaxBuildersPerProject>::default(),
								);
							devs.try_push(user.clone())
								.map_err(|_| Error::<T>::MaxBuildersPerProjectReached)?;
						},
					}
					Ok(())
			...
			ProxyRole::Investor => {
				...
				<ProjectsInfo<T>>::try_mutate::<_, _, DispatchError, _>(project_id, |project| {
					...
					match project.investor.as_mut() {
						...
						None => {
							let investors = project.investor.get_or_insert(BoundedVec::<
								T::AccountId,
								T::MaxInvestorsPerProject,
							>::default());
							investors
								.try_push(user.clone())
								.map_err(|_| Error::<T>::MaxInvestorsPerProjectReached)?;
					...
					match project.issuer.as_mut() {
						...
						None => {
							let issuers = project
								.issuer
								.get_or_insert(
									BoundedVec::<T::AccountId, T::MaxIssuersPerProject>::default(),
								);
							issuers
								.try_push(user.clone())
								.map_err(|_| Error::<T>::MaxIssuersPerProjectReached)?;
			...
			ProxyRole::RegionalCenter => {
				...
				<ProjectsInfo<T>>::try_mutate::<_, _, DispatchError, _>(project_id, |project| {
					...
					match project.regional_center.as_mut() {
						...
						None => {
							let regional_centers =
								project.regional_center.get_or_insert(BoundedVec::<
									T::AccountId,
	...
	fn is_project_completed(project_id: ProjectId) -> DispatchResult {
		...
		// Ensure project is not completed
			...
			project_data.status != ProjectStatus::Completed,
			Error::<T>::ProjectIsAlreadyCompleted
		);
	...
	pub fn is_authorized(
		...
		// Get user data
		let user_data = <UsersInfo<T>>::try_get(authority.clone())
			.map_err(|_| Error::<T>::UserNotRegistered)?;
		...
		match drawdown_data.drawdown_type {
			DrawdownType::EB5 => {
				...
				<ProjectsInfo<T>>::try_mutate::<_, _, DispatchError, _>(
					...
					|project_data| {
						let project_data =
							project_data.as_mut().ok_or(Error::<T>::ProjectNotFound)?;
	...
	pub fn do_reset_allen_drawdowns(project_id: ProjectId) -> DispatchResult {
		...
		let drawdowns = <DrawdownsByProject<T>>::try_get(project_id)
			.map_err(|_| Error::<T>::ProjectNotFound)?;
		...
		for drawdown_id in developer_equity_drawdowns.iter().cloned() {
			...
			<DrawdownsByProject<T>>::try_mutate::<_, _, DispatchError, _>(
				project_id,
				|drawdowns| {
