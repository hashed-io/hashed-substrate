...
pub type UsersAssignation<T> =
  BoundedVec<(AccountIdOf<T>, ProxyRole, AssignAction), <T as Config>::MaxRegistrationsAtTime>;
pub type Banks<T> = BoundedVec<(BankName, BankAddress), <T as Config>::MaxBanksPerProject>;
pub type PrivateGroupId = BoundedVec<u8, ConstU32<400>>;
pub type InflationRate = u32;
pub type ProjectsInflation<T> =
  BoundedVec<(ProjectId, Option<InflationRate>, CUDAction), <T as Config>::MaxRegistrationsAtTime>;

// Users
pub type DateRegistered = u64;
pub type Users<T> = BoundedVec<
  (AccountIdOf<T>, Option<FieldName>, Option<ProxyRole>, CUDAction),
...
pub type Transactions<T> = BoundedVec<
  ...
  <T as Config>::MaxRegistrationsAtTime,
>;
pub type TransactionsFeedback<T> =
  BoundedVec<(TransactionId, FieldDescription), <T as Config>::MaxRegistrationsAtTime>;

// Drawdowns
pub type DrawdownId = [u8; 32];
pub type DrawdownNumber = u32;
pub type DrawdownStatusChanges<T> =
  BoundedVec<(DrawdownStatus, UpdatedDate), <T as Config>::MaxStatusChangesPerDrawdown>;
pub type RecoveryRecord<T> =
  BoundedVec<(AccountIdOf<T>, UpdatedDate), <T as Config>::MaxRecoveryChanges>;
...
pub type RevenueStatusChanges<T> =
  BoundedVec<(RevenueStatus, UpdatedDate), <T as Config>::MaxStatusChangesPerRevenue>;
