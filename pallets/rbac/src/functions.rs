...
impl<T: Config> RoleBasedAccessControl<T::AccountId> for Pallet<T> {
	...
	fn remove_scope(pallet: IdOrVec, scope_id: ScopeId) -> DispatchResult {
		...
		<Scopes<T>>::try_mutate_exists::<_, (), DispatchError, _>(pallet_id, |scopes_option| {
			...
			let s_pos =
				scopes.iter().position(|&s| s == scope_id).ok_or(Error::<T>::ScopeNotFound)?;
			scopes.remove(s_pos);
			if scopes.is_empty() {
	...
	fn remove_pallet_storage(pallet: IdOrVec) -> DispatchResult {
		...
		let flatten_all_pallet_roles =
			all_pallet_roles.iter().flatten().collect::<Vec<&[u8; 32]>>();
	...
	fn remove_role_from_user(
		...
		<RolesByUser<T>>::try_mutate_exists::<_, (), DispatchError, _>(
			...
			|user_roles_option| {
				...
				let r_pos = user_roles
					.iter()
					.position(|&r| r == role_id)
					.ok_or(Error::<T>::RoleNotFound)?;
		...
		<UsersByScope<T>>::try_mutate_exists::<_, (), DispatchError, _>(
			...
			|auth_users_option| {
				...
				let u_pos =
					auth_users.iter().position(|u| *u == user).ok_or(Error::<T>::UserNotFound)?;
	...
	fn create_and_set_permissions(
		...
		for permission in permissions {
			permission_ids
				.push(Self::create_permission(pallet_id_enum.clone(), permission.to_owned())?);
		}
	...
	///
	/// Checks if the user has at least one role on the given scope.
	/// ### Parameters:
	/// - `user`: The account to validate.
	/// - `pallet_id`: The unique pallet identifier.
	/// - `scope_id`: The scope context in which the permission will be validated.
	fn has_any_role(account: T::AccountId, pallet: IdOrVec, scope_id: &ScopeId) -> bool {
		<RolesByUser<T>>::contains_key((account, pallet.to_id(), scope_id))
