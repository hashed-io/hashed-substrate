...
fn create_custodian_fields( custodian_account: u64, n_files: u32, ) ->
...
fn duplicate_marketplaces_shouldnt_work() {
	new_test_ext().execute_with(|| {
		// Dispatch a signed extrinsic.
		assert_ok!(GatedMarketplace::create_marketplace(RuntimeOrigin::signed(1),2, create_label("my marketplace") ));
		assert_noop!(GatedMarketplace::create_marketplace(RuntimeOrigin::signed(1),2, create_label("my marketplace") ), Error::<Test>::MarketplaceAlreadyExists);
	});
}
...
fn exceeding_max_roles_per_auth_shouldnt_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(GatedMarketplace::create_marketplace(RuntimeOrigin::signed(1),2, m_label.clone() ));
		assert_ok!(GatedMarketplace::add_authority(RuntimeOrigin::signed(1), 2, MarketplaceRole::Appraiser, m_label.using_encoded(blake2_256)));
		assert_noop!(
			GatedMarketplace::add_authority(RuntimeOrigin::signed(1), 2, MarketplaceRole::RedemptionSpecialist, m_label.using_encoded(blake2_256) ),
...
fn apply_to_marketplace_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(GatedMarketplace::create_marketplace(RuntimeOrigin::signed(1),2, create_label("my marketplace") ));
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert_ok!(GatedMarketplace::apply(RuntimeOrigin::signed(3),m_id, create_application_fields(2), None ));
...
fn apply_with_custodian_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(GatedMarketplace::create_marketplace(RuntimeOrigin::signed(1),2, create_label("my marketplace") ));
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert_ok!(GatedMarketplace::apply(RuntimeOrigin::signed(3),m_id, create_application_fields(2), create_custodian_fields(4,2) ));
...
fn apply_with_same_account_as_custodian_shouldnt_work() {
	new_test_ext().execute_with(|| {
		...
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert_noop!(
			GatedMarketplace::apply(RuntimeOrigin::signed(3),m_id, create_application_fields(2), create_custodian_fields(3,2) ),
...
fn exceeding_max_applications_per_custodian_shouldnt_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(GatedMarketplace::create_marketplace(RuntimeOrigin::signed(1),2, create_label("my marketplace") ));
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert_ok!(GatedMarketplace::apply(RuntimeOrigin::signed(3),m_id, create_application_fields(2), create_custodian_fields(6,2) ));
		assert_ok!(GatedMarketplace::apply(RuntimeOrigin::signed(4),m_id, create_application_fields(2), create_custodian_fields(6,2) ));
		assert_noop!(
			GatedMarketplace::apply(RuntimeOrigin::signed(5),m_id, create_application_fields(2), create_custodian_fields(6,2) ),
...
fn apply_to_nonexistent_marketplace_shouldnt_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(GatedMarketplace::apply(RuntimeOrigin::signed(3),m_id,create_application_fields(2), None ), Error::<Test>::MarketplaceNotFound);
	});
}
...
fn apply_twice_shouldnt_work() {
	new_test_ext().execute_with(|| {
		...
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert_ok!(GatedMarketplace::apply(RuntimeOrigin::signed(3), m_id, create_application_fields(2), None ));
		assert_noop!(GatedMarketplace::apply(RuntimeOrigin::signed(3),m_id, create_application_fields(2), None ), Error::<Test>::AlreadyApplied );
	});
}
...
fn exceeding_max_applicants_shouldnt_work() {
	new_test_ext().execute_with(|| {
		...
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert_ok!(GatedMarketplace::apply(RuntimeOrigin::signed(3),m_id, create_application_fields(2), None ));
		assert_ok!(GatedMarketplace::apply(RuntimeOrigin::signed(4),m_id, create_application_fields(3), None ));
		assert_ok!(GatedMarketplace::apply(RuntimeOrigin::signed(5),m_id, create_application_fields(3), None ));
		assert_noop!(GatedMarketplace::apply(RuntimeOrigin::signed(6),m_id, create_application_fields(1), None ), Error::<Test>::ExceedMaxApplicants );
	});
}
...
fn enroll_works() {
	new_test_ext().execute_with(|| {
		...
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert_ok!(GatedMarketplace::apply(RuntimeOrigin::signed(3),m_id,create_application_fields(2), None ));
		assert_ok!(GatedMarketplace::apply(RuntimeOrigin::signed(4),m_id,create_application_fields(1), None));
		let app_id = GatedMarketplace::applications_by_account(3,m_id).unwrap();
		// enroll with account
		assert_ok!(GatedMarketplace::enroll(RuntimeOrigin::signed(1), m_id , AccountOrApplication::Account(3), true, default_feedback()));
		// enroll with application
		assert_ok!(GatedMarketplace::enroll(RuntimeOrigin::signed(1), m_id , AccountOrApplication::Application(app_id), false, default_feedback()));
	});
}
...
fn enroll_rejected_works() {
	new_test_ext().execute_with(|| {
		...
		// reject with account
		assert_ok!(GatedMarketplace::enroll(RuntimeOrigin::signed(1), m_id , AccountOrApplication::Account(3), false, default_feedback()));
...
fn enroll_rejected_has_feedback_works() {
	new_test_ext().execute_with(|| {
		...
		let app_id = GatedMarketplace::applications_by_account(3,m_id).unwrap();
		// reject with account
		assert_ok!(GatedMarketplace::enroll(RuntimeOrigin::signed(1), m_id , AccountOrApplication::Account(3), true, feedback("We need to accept this application")));
		// reject with application
		assert_ok!(GatedMarketplace::enroll(RuntimeOrigin::signed(1), m_id , AccountOrApplication::Application(app_id), false, feedback("We need to reject this application")));
...
fn enroll_approved_has_feedback_works() {
	new_test_ext().execute_with(|| {
		...
		let app_id = GatedMarketplace::applications_by_account(4,m_id).unwrap();
		// reject with account
		assert_ok!(GatedMarketplace::enroll(RuntimeOrigin::signed(1), m_id , AccountOrApplication::Account(3), true, feedback("We've rejected your publication")));
		// reject with application
		assert_ok!(GatedMarketplace::enroll(RuntimeOrigin::signed(1), m_id , AccountOrApplication::Application(app_id), true, feedback("We've rejected your publication")));
...
fn change_enroll_status_works() {
	new_test_ext().execute_with(|| {
		...
		let app_id = GatedMarketplace::applications_by_account(4,m_id).unwrap();
		// reject an account
		assert_ok!(GatedMarketplace::enroll(RuntimeOrigin::signed(1), m_id , AccountOrApplication::Account(4), false, default_feedback()));
		// and then change it to "accepted"
		assert_ok!(GatedMarketplace::enroll(RuntimeOrigin::signed(1), m_id , AccountOrApplication::Application(app_id), true, default_feedback()));
	});
}
...
fn non_authorized_user_enroll_shouldnt_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(GatedMarketplace::enroll(RuntimeOrigin::signed(4), m_id , AccountOrApplication::Account(3), true, default_feedback()), RbacErr::NotAuthorized);
	});
}
...
fn enroll_nonexistent_application_shouldnt_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(GatedMarketplace::enroll(RuntimeOrigin::signed(1), m_id , AccountOrApplication::Account(3), true, default_feedback()), Error::<Test>::ApplicationNotFound);
		// accept nonexisten application throws error (application id version)
		assert_noop!(GatedMarketplace::enroll(RuntimeOrigin::signed(1), m_id , AccountOrApplication::Application([0;32]), true, default_feedback()), Error::<Test>::ApplicationNotFound);
	});
}
...
fn add_authority_appraiser_works() {
	new_test_ext().execute_with(|| {
		assert_ok!(GatedMarketplace::create_marketplace(RuntimeOrigin::signed(1),2, create_label("my marketplace") ));
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert_ok!(GatedMarketplace::add_authority(RuntimeOrigin::signed(1), 3, MarketplaceRole::Appraiser, m_id));
...
fn add_authority_admin_works() {
	new_test_ext().execute_with(|| {
		assert_ok!(GatedMarketplace::create_marketplace(RuntimeOrigin::signed(1),2, create_label("my marketplace") ));
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert_ok!(GatedMarketplace::add_authority(RuntimeOrigin::signed(1), 3, MarketplaceRole::Admin, m_id));
...
fn add_authority_redenmption_specialist_works() {
	new_test_ext().execute_with(|| {
		assert_ok!(GatedMarketplace::create_marketplace(RuntimeOrigin::signed(1),2, create_label("my marketplace") ));
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert_ok!(GatedMarketplace::add_authority(RuntimeOrigin::signed(1), 3, MarketplaceRole::RedemptionSpecialist, m_id));
...
fn add_authority_owner_shouldnt_work() {
	new_test_ext().execute_with(|| {
		assert_ok!(GatedMarketplace::create_marketplace(RuntimeOrigin::signed(1),2, create_label("my marketplace") ));
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert_noop!(GatedMarketplace::add_authority(RuntimeOrigin::signed(1), 3, MarketplaceRole::Owner, m_id), Error::<Test>::OnlyOneOwnerIsAllowed);
...
fn add_authority_cant_apply_twice_shouldnt_work(){
	new_test_ext().execute_with(|| {
		...
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert_ok!(GatedMarketplace::add_authority(RuntimeOrigin::signed(1), 3, MarketplaceRole::Appraiser, m_id));
		assert_noop!(GatedMarketplace::add_authority(RuntimeOrigin::signed(1), 3, MarketplaceRole::Appraiser, m_id), RbacErr::UserAlreadyHasRole);
	});
}
...
fn remove_authority_appraiser_works() {
	new_test_ext().execute_with(|| {
		...
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert_ok!(GatedMarketplace::add_authority(RuntimeOrigin::signed(1), 3, MarketplaceRole::Appraiser, m_id));
		assert_ok!(GatedMarketplace::remove_authority(RuntimeOrigin::signed(1), 3, MarketplaceRole::Appraiser, m_id));
...
fn remove_authority_admin_works() {
	new_test_ext().execute_with(|| {
		...
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert_ok!(GatedMarketplace::add_authority(RuntimeOrigin::signed(1), 3, MarketplaceRole::Admin, m_id));
		assert_ok!(GatedMarketplace::remove_authority(RuntimeOrigin::signed(1), 3, MarketplaceRole::Admin, m_id));
		//assert!(GatedMarketplace::marketplaces_by_authority(3, m_id) == vec![]);
		assert!(RBAC::roles_by_user((3, pallet_id(), m_id)).is_empty());
	});
}
...
fn remove_authority_redemption_specialist_work() {
	new_test_ext().execute_with(|| {
		...
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert_ok!(GatedMarketplace::add_authority(RuntimeOrigin::signed(1), 3, MarketplaceRole::RedemptionSpecialist, m_id));
		assert_ok!(GatedMarketplace::remove_authority(RuntimeOrigin::signed(1), 3, MarketplaceRole::RedemptionSpecialist, m_id));
		//assert!(GatedMarketplace::marketplaces_by_authority(3, m_id) == vec![]);
		assert!(RBAC::roles_by_user((3, pallet_id(), m_id)).is_empty());
	});
}
...
fn remove_authority_owner_shouldnt_work(){
	new_test_ext().execute_with(|| {
		assert_ok!(GatedMarketplace::create_marketplace(RuntimeOrigin::signed(1),2, create_label("my marketplace") ));
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert_noop!(GatedMarketplace::remove_authority(RuntimeOrigin::signed(1), 1 , MarketplaceRole::Owner, m_id), Error::<Test>::CantRemoveOwner);
	});
}
...
fn remove_authority_admin_by_admin_shouldnt_work(){
	new_test_ext().execute_with(|| {
		...
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert_ok!(GatedMarketplace::add_authority(RuntimeOrigin::signed(1), 3, MarketplaceRole::Admin, m_id));
		assert_noop!(GatedMarketplace::remove_authority(RuntimeOrigin::signed(3), 3, MarketplaceRole::Admin, m_id), Error::<Test>::AdminCannotRemoveItself);
	});
}
...
fn remove_authority_user_tries_to_remove_non_existent_role_shouldnt_work(){
	new_test_ext().execute_with(|| {
		...
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert_ok!(GatedMarketplace::add_authority(RuntimeOrigin::signed(1), 3, MarketplaceRole::Appraiser, m_id));
		assert_noop!(GatedMarketplace::remove_authority(RuntimeOrigin::signed(1), 3, MarketplaceRole::Admin, m_id), RbacErr::RoleNotFound);
	});
}
...
fn remove_authority_user_is_not_admin_or_owner_shouldnt_work(){
	new_test_ext().execute_with(|| {
		...
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert_ok!(GatedMarketplace::add_authority(RuntimeOrigin::signed(1), 3, MarketplaceRole::Appraiser, m_id));
		assert_ok!(GatedMarketplace::add_authority(RuntimeOrigin::signed(1), 4, MarketplaceRole::Admin, m_id));
		assert_noop!(GatedMarketplace::remove_authority(RuntimeOrigin::signed(3), 4,MarketplaceRole::Appraiser, m_id), RbacErr::NotAuthorized);
	});
}
...
fn update_marketplace_marketplace_not_found_shouldnt_work(){
	new_test_ext().execute_with(|| {
		...
		assert_noop!(GatedMarketplace::update_label_marketplace(RuntimeOrigin::signed(1), m_id_2, create_label("my marketplace 2")), Error::<Test>::MarketplaceNotFound);
	});
...
fn update_marketplace_user_without_permission_shouldnt_work(){
	new_test_ext().execute_with(|| {
		...
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert_ok!(GatedMarketplace::add_authority(RuntimeOrigin::signed(1), 3, MarketplaceRole::Appraiser, m_id));
		assert_noop!(GatedMarketplace::update_label_marketplace(RuntimeOrigin::signed(3), m_id, create_label("my marketplace2")), RbacErr::NotAuthorized);
	});
}
...
fn update_label_marketplace_works(){
	new_test_ext().execute_with(|| {
		...
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert!(GatedMarketplace::marketplaces(m_id).is_some());
		assert_ok!(GatedMarketplace::update_label_marketplace(RuntimeOrigin::signed(1), m_id, create_label("my marketplace 2")));
		assert!(GatedMarketplace::marketplaces(m_id).is_some() );
	});
}
...
fn remove_marketplace_marketplace_not_found_shouldnt_work(){
	new_test_ext().execute_with(|| {
		...
		assert!(GatedMarketplace::marketplaces(m_id).is_some());
		let m_id_2 = create_label("not the first marketplace").using_encoded(blake2_256);
		assert_noop!(GatedMarketplace::remove_marketplace(RuntimeOrigin::signed(1), m_id_2), Error::<Test>::MarketplaceNotFound);
	});
...
fn remove_marketplace_user_without_permission_shouldnt_work(){
	new_test_ext().execute_with(|| {
		...
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert_ok!(GatedMarketplace::add_authority(RuntimeOrigin::signed(1), 3, MarketplaceRole::Appraiser, m_id));
		assert_noop!(GatedMarketplace::remove_marketplace(RuntimeOrigin::signed(3), m_id), RbacErr::NotAuthorized);
	});
}
...
fn remove_marketplace_deletes_storage_from_marketplaces_by_authority_works(){
	new_test_ext().execute_with(|| {
		...
		assert_ok!(GatedMarketplace::add_authority(RuntimeOrigin::signed(1), 3, MarketplaceRole::Appraiser, m_id));
		assert_ok!(GatedMarketplace::add_authority(RuntimeOrigin::signed(1), 4, MarketplaceRole::RedemptionSpecialist, m_id));
...
fn reapply_user_has_never_applied_shouldnt_work(){
	new_test_ext().execute_with(|| {
		...
		let m_id = create_label("my marketplace").using_encoded(blake2_256);
		assert!(GatedMarketplace::marketplaces(m_id).is_some());
		assert_noop!(GatedMarketplace::reapply(RuntimeOrigin::signed(3), m_id, create_application_fields(2), None), Error::<Test>::ApplicationIdNotFound);
	});
}
...
fn reapply_with_wrong_marketplace_id_shouldnt_work() {
	new_test_ext().execute_with(|| {
		...
		assert!(GatedMarketplace::marketplaces(m_id).is_some());
		assert_noop!(GatedMarketplace::reapply(RuntimeOrigin::signed(1), m_id2, create_application_fields(2), None), Error::<Test>::ApplicationIdNotFound);
	});
}
...
fn reapply_status_application_is_still_pendding_shouldnt_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(GatedMarketplace::reapply(RuntimeOrigin::signed(3), m_id, create_application_fields(2), None), Error::<Test>::ApplicationStatusStillPending);
	});
}
...
fn reapply_status_application_is_already_approved_shouldnt_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(GatedMarketplace::reapply(RuntimeOrigin::signed(3), m_id, create_application_fields(2), None), Error::<Test>::ApplicationHasAlreadyBeenApproved);
	});
}
...
fn reapply_works() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(GatedMarketplace::applications(app_id).unwrap().status, ApplicationStatus::Pending);
		assert_ok!(GatedMarketplace::enroll(RuntimeOrigin::signed(1), m_id , AccountOrApplication::Application(app_id), false, default_feedback()));
		assert_ok!(GatedMarketplace::reapply(RuntimeOrigin::signed(3), m_id, create_application_fields(2), None));
...
fn enlist_sell_offer_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(GatedMarketplace::enlist_sell_offer(RuntimeOrigin::signed(1), m_id, 0, 0, 10000));
...
fn enlist_sell_offer_item_does_not_exist_shouldnt_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(GatedMarketplace::enlist_sell_offer(RuntimeOrigin::signed(1), m_id, 0, 1, 10000), Error::<Test>::CollectionNotFound);
	});
}
...
fn enlist_sell_offer_item_already_enlisted_shouldnt_work() {
	new_test_ext().execute_with(|| {
		...
		let offer_id = GatedMarketplace::offers_by_item(0, 0).iter().next().unwrap().clone();
		assert!(GatedMarketplace::offers_info(offer_id).is_some());
		assert_noop!(GatedMarketplace::enlist_sell_offer(RuntimeOrigin::signed(1), m_id, 0, 0, 10000), Error::<Test>::OfferAlreadyExists);
	});
}
...
fn enlist_sell_offer_not_owner_tries_to_enlist_shouldnt_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(GatedMarketplace::enlist_sell_offer(RuntimeOrigin::signed(2), m_id, 0, 0, 10000), Error::<Test>::NotOwner);
	});
}
...
fn enlist_sell_offer_price_must_greater_than_zero_shouldnt_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(GatedMarketplace::enlist_sell_offer(RuntimeOrigin::signed(1), m_id, 0, 0, 0), Error::<Test>::PriceMustBeGreaterThanZero);
	});
}
...
fn enlist_sell_offer_price_must_greater_than_minimun_amount_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(GatedMarketplace::enlist_sell_offer(RuntimeOrigin::signed(1), m_id, 0, 0, minimum_amount));
		let offer_id = GatedMarketplace::offers_by_item(0, 0).iter().next().unwrap().clone();
		assert!(GatedMarketplace::offers_info(offer_id).is_some());
	});
}
...
fn enlist_sell_offer_two_marketplaces(){
	new_test_ext().execute_with(|| {
		...
		assert_ok!(GatedMarketplace::create_marketplace(RuntimeOrigin::signed(1), 2, create_label("my marketplace2")));
		let m_id2 = create_label("my marketplace2").using_encoded(blake2_256);
		...
		assert_ok!(GatedMarketplace::enlist_sell_offer(RuntimeOrigin::signed(1), m_id, 0, 0, 10000));
		assert_ok!(GatedMarketplace::enlist_sell_offer(RuntimeOrigin::signed(1), m_id2, 0, 0, 11000));
...
fn enlist_buy_offer_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(GatedMarketplace::enlist_sell_offer(RuntimeOrigin::signed(1), m_id, 0, 0, 1001));
		let offer_id = GatedMarketplace::offers_by_account(1).iter().next().unwrap().clone();
		assert!(GatedMarketplace::offers_info(offer_id).is_some());

		assert_ok!(GatedMarketplace::enlist_buy_offer(RuntimeOrigin::signed(2), m_id, 0, 0, 1100));
...
fn enlist_buy_offer_item_is_not_for_sale_shouldnt_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(GatedMarketplace::enlist_buy_offer(RuntimeOrigin::signed(2), m_id, 0, 1, 1100), Error::<Test>::ItemNotForSale);
	});
}
...
fn enlist_buy_offer_owner_cannnot_create_buy_offers_for_their_own_items_shouldnt_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(GatedMarketplace::enlist_buy_offer(RuntimeOrigin::signed(1), m_id, 0, 0, 1100), Error::<Test>::CannotCreateOffer);
	});
}
...
fn enlist_buy_offer_user_does_not_have_enough_balance_shouldnt_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(GatedMarketplace::enlist_buy_offer(RuntimeOrigin::signed(2), m_id, 0, 0, 10000), Error::<Test>::NotEnoughBalance);
...
fn enlist_buy_offer_price_must_greater_than_zero_shouldnt_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(GatedMarketplace::enlist_buy_offer(RuntimeOrigin::signed(2), m_id, 0, 0, 0), Error::<Test>::PriceMustBeGreaterThanZero);
	});
}
...
fn enlist_buy_offer_an_item_can_receive_multiple_buy_offers(){
	new_test_ext().execute_with(|| {
		...
		assert_ok!(GatedMarketplace::apply(RuntimeOrigin::signed(3),m_id,create_application_fields(2), None ));
		assert_ok!(GatedMarketplace::enroll(RuntimeOrigin::signed(1), m_id , AccountOrApplication::Account(3), true, default_feedback()));
		assert_ok!(GatedMarketplace::enlist_buy_offer(RuntimeOrigin::signed(3), m_id, 0, 0, 1200));
...
fn take_sell_offer_works(){
	new_test_ext().execute_with(|| {
		...
		assert_ok!(GatedMarketplace::enlist_sell_offer(RuntimeOrigin::signed(1), m_id, 0, 0, 1200));
		let offer_id = GatedMarketplace::offers_by_item(0, 0).iter().next().unwrap().clone();
...
fn take_sell_offer_owner_cannnot_be_the_buyer_shouldnt_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(GatedMarketplace::take_sell_offer(RuntimeOrigin::signed(1), offer_id), Error::<Test>::CannotTakeOffer);
	});
}
...
fn take_sell_offer_id_does_not_exist_shouldnt_work(){
	new_test_ext().execute_with(|| {
		...
		assert_noop!(GatedMarketplace::take_sell_offer(RuntimeOrigin::signed(2), offer_id2), Error::<Test>::OfferNotFound);
	});
}
...
fn take_sell_offer_buyer_does_not_have_enough_balance_shouldnt_work(){
	new_test_ext().execute_with(|| {
		...
		assert_noop!(GatedMarketplace::take_sell_offer(RuntimeOrigin::signed(2), offer_id), Error::<Test>::NotEnoughBalance);
	});
}
...
fn take_buy_offer_works(){
	new_test_ext().execute_with(|| {
		...
		assert_ok!(GatedMarketplace::enlist_buy_offer(RuntimeOrigin::signed(2), m_id, 0, 0, 1200));
		let offer_id2 = GatedMarketplace::offers_by_account(2).iter().next().unwrap().clone();
		assert_eq!(GatedMarketplace::offers_info(offer_id2).unwrap().offer_type, OfferType::BuyOrder);

		assert_ok!(GatedMarketplace::take_buy_offer(RuntimeOrigin::signed(1), offer_id2));
		assert_eq!(GatedMarketplace::offers_by_item(0, 0).len(), 0);
		assert_eq!(GatedMarketplace::offers_info(offer_id).unwrap().status, OfferStatus::Closed);
	});
}
...
fn take_buy_offer_only_owner_can_accept_buy_offers_shouldnt_work(){
	new_test_ext().execute_with(|| {
		...
		assert_noop!(GatedMarketplace::take_buy_offer(RuntimeOrigin::signed(2), offer_id2), Error::<Test>::NotOwner);
	});
}
...
fn take_buy_offer_id_does_not_exist_shouldnt_work(){
	new_test_ext().execute_with(|| {
		...
		assert_noop!(GatedMarketplace::take_buy_offer(RuntimeOrigin::signed(1), offer_id3), Error::<Test>::OfferNotFound);
...
fn take_buy_offer_user_does_not_have_enough_balance_shouldnt_work(){
	new_test_ext().execute_with(|| {
		...
		assert_noop!(GatedMarketplace::take_buy_offer(RuntimeOrigin::signed(1), offer_id2), Error::<Test>::NotEnoughBalance);
	});
}
...
fn remove_sell_offer_works(){
	new_test_ext().execute_with(|| {
		...
		assert_ok!(GatedMarketplace::remove_offer(RuntimeOrigin::signed(1), offer_id));
...
fn remove_buy_offer_works(){
	new_test_ext().execute_with(|| {
		...
		assert_ok!(GatedMarketplace::enlist_buy_offer(RuntimeOrigin::signed(2), m_id, 0, 0, 1001));
		let offer_id2 = GatedMarketplace::offers_by_account(2).iter().next().unwrap().clone();
		assert!(GatedMarketplace::offers_info(offer_id2).is_some());

		assert_ok!(GatedMarketplace::remove_offer(RuntimeOrigin::signed(2), offer_id2));
...
fn remove_offer_id_does_not_exist_sholdnt_work(){
	new_test_ext().execute_with(|| {
		...
		let offer_id2 = offer_id.using_encoded(blake2_256);
		assert_noop!(GatedMarketplace::remove_offer(RuntimeOrigin::signed(2), offer_id2), Error::<Test>::OfferNotFound);
	});
}
...
fn remove_offer_creator_doesnt_match_sholdnt_work(){
	new_test_ext().execute_with(|| {
		...
		assert_noop!(GatedMarketplace::remove_offer(RuntimeOrigin::signed(2), offer_id), Error::<Test>::CannotRemoveOffer);
	});
}
...
fn remove_offer_status_is_closed_shouldnt_work(){
	new_test_ext().execute_with(|| {
		...
		assert_noop!(GatedMarketplace::remove_offer(RuntimeOrigin::signed(2), offer_id2), Error::<Test>::CannotDeleteOffer);
	});
