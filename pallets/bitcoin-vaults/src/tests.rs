...
fn creating_vault_should_work() {
  new_test_ext().execute_with(|| {
    ...
    let cosigners =
      BoundedVec::<<Test as frame_system::Config>::AccountId, MaxCosignersPerVault>::try_from(
        [test_pub(2)].to_vec(),
      )
      .unwrap();
...
fn exceeding_max_cosigners_per_vault_should_work() {
  new_test_ext().execute_with(|| {
    ...
    let cosigners =
      BoundedVec::<<Test as frame_system::Config>::AccountId, MaxCosignersPerVault>::try_from(
        [test_pub(2), test_pub(3), test_pub(4)].to_vec(),
      )
      .unwrap();
    assert_noop!(
      BitcoinVaults::create_vault(
...
fn save_nonexistent_proof_psbt_should_not_work() {
  new_test_ext().execute_with(|| {
    ...
    assert_noop!(
      BitcoinVaults::save_proof_psbt(RuntimeOrigin::signed(test_pub(1)), vault_id, dummy_psbt(),),
...
fn finalize_proof_psbt_should_work() {
  new_test_ext().execute_with(|| {
    ...
    assert_eq!(ProofOfReserves::<Test>::get(vault_id).unwrap().status, ProposalStatus::Broadcasted)
  });
}
...
fn finalize_proof_twice_should_not_work() {
  new_test_ext().execute_with(|| {
    ...
    assert_noop!(
      BitcoinVaults::finalize_proof(RuntimeOrigin::signed(test_pub(2)), vault_id, dummy_psbt()),
